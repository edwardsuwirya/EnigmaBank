        ...
        CancellationToken cancellationToken)
    {
        ...
        if (accountInDb is null)
            return await Task.FromResult(
                new ResponseWrapper<AccountResponse>().Fail(ExistenceErrors.NotFound(request.AccountNumber)));

        return await Task.FromResult(new ResponseWrapper<AccountResponse>().Success(
